...
svr = GridSearchCV(SVR(), param_grid=param_grid_svr, cv=5, verbose=0)
...
param_grid_dt = dict(min_samples_leaf=[25, 30],
                     max_depth=[1000, 2000],
                     min_impurity_decrease=[0.15, 0.17],
                     )

dt = GridSearchCV(DecisionTreeRegressor(), param_grid_dt, cv=10)
...
param_grid_rf = dict(n_estimators=[150, 125],
                     max_depth=[1, 5],
...
forest = GridSearchCV(RandomForestRegressor(
    random_state=0), param_grid_rf, cv=5)
